"""
Train and test with Logistic Regression, Random Forest and Multi-layer perception (with property vector)

Input:
    --test_name: T2D, Limaye, Wikipedia
    --use_surrounding_columns (include the word2vec or number of surrounding columns in the input)
    --use_property_vector (set it to yes if it's to test property vector)
    --prop2vec_dim: (number of candidate properties)

Output:
    column-level accuracy will be printed
    the score will be output if --output_score is set to 'yes'
"""
import os
import sys
import json
...
parser.add_argument('--use_surrounding_columns', type=str, default='no', help='yes or no')
...
parser.add_argument('--output_score', type=str, default='no', help='yes or no')
...
def save_scores(scores, cls_name, train_or_test='test'):
    if FLAGS.output_score:
        save_dir = os.path.join(FLAGS.io_dir, '%s_%s_%s' % (train_or_test, FLAGS.algorithm, FLAGS.test_name))
...
# output FC layer for the training samples
if FLAGS.output_score:
    print '\n generating training scores ...'
    ...
    for cls in cls_cols:
        ...
        col_prop2vecs = None
        if FLAGS.use_property_vector:
            vec_file = os.path.join(FLAGS.io_dir, 'train_prop2vecs', '%s.vec' % cls)
            col_prop2vecs = pickle.load(open(vec_file))
